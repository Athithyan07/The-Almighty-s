...
export async function POST(req: Request) {
  try {
    ...
    // Create Transporter (uses Gmail App Password if configured, or console logger)
    ...
    /* ── 1. SEND EMAIL TO SCHOOL (godalmightyschool03@gmail.com) ── */
    ...
    /* ── 2. SEND CONFIRMATION EMAIL TO USER (If contact is email) ── */
    ...
    /* ── 3. SMS DISPATCH (Twilio / Fast2SMS Integration) ── */
    let smsStatus = "skipped";
    if (isPhone && phoneDigits) {
      // Fast2SMS API (Indian SMS Gateway) or Twilio API
      const fast2smsKey = process.env.FAST2SMS_API_KEY;
      const twilioSid = process.env.TWILIO_ACCOUNT_SID;
      const twilioToken = process.env.TWILIO_AUTH_TOKEN;
      const twilioFrom = process.env.TWILIO_PHONE_NUMBER;

      const smsText = `Dear ${name}, thank you for inquiring about admission at The Almighty's Matriculation School (${grade}). Our team will call you shortly on ${phoneDigits}. Helpline: +91 8098062321`;
      ...
      if (fast2smsKey) {
        ...
        smsStatus = "mock_logged";
        console.log(`[SMS DISPATCH MOCK] To: ${phoneDigits} | Message: ${smsText}`);
      }
    }

    return NextResponse.json({
      success: true,
      message: "Inquiry processed successfully. Notifications dispatched to school & applicant.",
      data: {
        name,
        contact,
        grade,

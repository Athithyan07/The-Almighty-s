    ...
    >
      <div>
        <div className="text-3xl sm:text-4xl lg:text-5xl font-extrabold font-heading tracking-tight text-slate-900 dark:text-white flex items-baseline">
          <span className="text-[#00A3E0] mr-1">{prefix}</span>
          <span className="bg-gradient-to-r from-slate-900 via-[#00A3E0] to-slate-900 dark:from-white dark:via-sky-100 dark:to-sky-200 bg-clip-text text-transparent">
            {decimals > 0 ? count.toFixed(decimals) : Math.floor(count).toLocaleString()}
          </span>
          <span className="text-[#00A3E0] ml-1">{suffix}</span>
        </div>
        <h4 className="mt-2 text-base sm:text-lg font-bold text-slate-800 dark:text-slate-100 group-hover:text-[#00A3E0] dark:group-hover:text-[#33B8E8] transition-colors">
          {label}
        </h4>
      </div>
      {description && (
